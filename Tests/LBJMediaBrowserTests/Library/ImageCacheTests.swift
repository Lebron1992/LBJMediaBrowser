...
// Refers to https://github.com/Alamofire/AlamofireImage/blob/master/Tests/ImageCacheTests.swift

final class ImageCacheTests: BaseTestCase {
  var cache: AutoPurgingPHAssetImageCache!
  ...
  override func setUp() {
    ...
    cache = {
      ...
      return AutoPurgingPHAssetImageCache(memoryCapacity: memoryCapacity,
  ...
  func testThatCacheCanBeInitializedAndDeinitialized() {
    // Given
    var cache: AutoPurgingPHAssetImageCache? = AutoPurgingPHAssetImageCache()
  ...
  func testThatItCanAddImageToCacheWithRequestIdentifier() {
    // Given
    let image = self.image(forResource: "unicorn", withExtension: "png")
    let request = PHAssetImageRequest.template
    let identifier = "-unicorn"
...
final class ImageCacheThreadSafetyTests: BaseTestCase {
  func testMultipleReadsAndWrites() {
    // Given
    let cache = AutoPurgingPHAssetImageCache(memoryCapacity: 1_000_000, preferredMemoryUsageAfterPurge: 100_000)
    let image = self.image(forResource: "unicorn", withExtension: "png")
